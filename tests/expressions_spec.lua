...
describe("expression argument parsing", function()
    ...
    it("keeps a comma literal when it is backslash-escaped", function()
    ...
    it("unescapes a doubled backslash to a single literal backslash", function()
    ...
    it("keeps a lone backslash literal before a non-escapable char", function()
        register_nargs("nargs4c2")
        -- '\n' is not an escape sequence: the backslash survives verbatim.
        local ok, res = resolve({ x = [[${nargs4c2:a\nb}]] })
        assert.is_true(ok)
        assert.are.equal([[#1:a\nb]], res.x)
    end)

    it("does not re-split a nested expression's comma-bearing output", function()
        register_nargs("nargs4d")
        expressions.register("withcomma2", function() return "x,y" end)
        -- A nested expression's output is never re-split, so no escaping is needed.
        local ok, res = resolve({ x = [[${nargs4d:<${withcomma2}>}]] })
        assert.is_true(ok)
        assert.are.equal("#1:<x,y>", res.x)
    end)

    it("splits on an unescaped comma", function()
...
describe("top-level escaping", function()
    it("treats an escaped dollar-brace as literal text, not an expression", function()
        local ok, res = resolve({ x = [[price is \${5}]] })
        assert.is_true(ok)
        assert.are.equal("price is ${5}", res.x)
    end)

    it("no longer treats $$ as an escape (it is literal)", function()
        local ok, res = resolve({ x = [[a$$b]] })
        assert.is_true(ok)
        assert.are.equal("a$$b", res.x)
    end)

    it("keeps a lone backslash before ordinary text", function()
        local ok, res = resolve({ x = [[C:\Users\me]] })
        assert.is_true(ok)
        assert.are.equal([[C:\Users\me]], res.x)
    end)

    it("unescapes a doubled backslash", function()
        local ok, res = resolve({ x = [[a\\b]] })
        assert.is_true(ok)
        assert.are.equal([[a\b]], res.x)
    end)

    it("still expands after an escaped dollar", function()
        expressions.register("who", function() return "bob" end)
        local ok, res = resolve({ x = [[\$${who}]] })
        assert.is_true(ok)
        assert.are.equal("$bob", res.x)
    end)
end)

describe("brace handling & escaping", function()
    local function register_echo(name)
        expressions.register(name, function(_, a) return a end)
    end

    it("passes a lone opening brace through unescaped", function()
        register_echo("br1")
        -- A `{` not part of a `${` needs no escaping.
        local ok, res = resolve({ x = "${br1:a{b}" })
        assert.is_true(ok)
        assert.are.equal("a{b", res.x)
    end)

    it("keeps a closing brace literal when escaped", function()
        register_echo("br2")
        local ok, res = resolve({ x = [[${br2:a\}b}]] })
        assert.is_true(ok)
        assert.are.equal("a}b", res.x)
    end)

    it("keeps a literal dollar-brace via an escaped dollar", function()
        register_echo("br3")
        local ok, res = resolve({ x = [[${br3:\${x\}}]] })
        assert.is_true(ok)
        assert.are.equal("${x}", res.x)
    end)

    it("leaves quotes literal (shell quotes pass through untouched)", function()
        register_echo("br5")
        local ok, res = resolve({ x = [[${br5:printf "hi"}]] })
        assert.is_true(ok)
        assert.are.equal([[printf "hi"]], res.x)
    end)

    it("leaves a lone apostrophe literal", function()
        register_echo("br6")
        local ok, res = resolve({ x = "${br6:don't}" })
        assert.is_true(ok)
        assert.are.equal("don't", res.x)
    end)

    it("still expands a nested expression beside an escaped brace", function()
        register_echo("br7")
        expressions.register("brinner", function() return "X" end)
        local ok, res = resolve({ x = [[${br7:\}${brinner}}]] })
        assert.is_true(ok)
        assert.are.equal("}X", res.x)
    end)

    it("errors clearly on an unterminated expression", function()
        register_echo("br8")
...
describe("lua builtin", function()
    ...
    it("errors when a result is not a scalar", function()
        -- `os` is a table; a literal `{1, 2}` would need its brace escaped.
