...
describe("brace handling & quoting", function()
    local function register_echo(name)
        expressions.register(name, function(_, a) return a end)
    end

    it("passes balanced braces through unquoted", function()
        register_echo("br1")
        local ok, res = resolve({ x = "${br1:a{b}c}" })
        assert.is_true(ok)
        assert.are.equal("a{b}c", res.x)
    end)

    it("protects an unbalanced } with quotes", function()
        register_echo("br2")
        local ok, res = resolve({ x = [[${br2:"}"}]] })
        assert.is_true(ok)
    ...
    it("protects an unbalanced { with quotes", function()
        register_echo("br3")
        local ok, res = resolve({ x = [[${br3:"{"}]] })
        assert.is_true(ok)
        assert.are.equal("{", res.x)
    end)

    it("protects a brace inside a whole-argument quote with doubled quotes", function()
        register_echo("br4")
        -- Quote the whole argument and double the inner quotes; the `}` is shielded.
        local ok, res = resolve({ x = [[${br4:"printf ""a}b"""}]] })
        assert.is_true(ok)
        assert.are.equal([[printf "a}b"]], res.x)
    end)

    it("leaves a mid-argument quote literal (shell quotes pass through)", function()
        register_echo("br5")
    ...
    it("still expands a nested expression inside a quoted brace-bearing arg", function()
        register_echo("br7")
