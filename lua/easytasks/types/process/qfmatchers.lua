...
local function item(file, lnum, col, text, type)
...
local function linter(line, _)
    ...
    if file then
        ...
        return item(file, lnum, col, msg .. " (" .. code .. ")", t)
    end
    ...
    if file2 then
        ...
        return item(file2, lnum2, col2, msg2, t)
    end
    local file3, lnum3, col3, msg3 = line:match("^([^%s:]+):(%d+):(%d+):%s*(.+)$")
    if file3 then return item(file3, lnum3, col3, msg3, "W") end
    return nil
end

--- Generic Unix format: file:line:col: message
---@type easytasks.QfMatcher
local function unix(line, _)
    local file, lnum, col, msg = line:match("^([^%s:]+):(%d+):(%d+):%s*(.*)$")
