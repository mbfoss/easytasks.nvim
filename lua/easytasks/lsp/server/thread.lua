-- easytasks LSP server — in-process alternative to server.lua.
...
-- module must not reference any easytasks.lsp.thread.thread_server file-level local.
local M = {}
...
function M.thread_main(read_fd, write_fd)
    ...
    require("easytasks.lsp.server.compact").install()

    local frame       = require("easytasks.lsp.server.frame")
    ...
    local hover       = require("easytasks.lsp.server.hover")
    local actions     = require("easytasks.lsp.server.actions")
    local doc_symbol  = require("easytasks.lsp.server.symbols")
    local fmt         = require("easytasks.lsp.server.format")
    ...
    local function log(msg, level)
        write_frame({
            ...
            params = { type = level or MessageType.Log, message = "[easytasks] " .. tostring(msg) },
        })
    end
    ...
    local function dispatch(msg)
        ...
        if method == "textDocument/didOpen" then
            ...
            doc_text[uri] = text
            -- Schema arrives via easytasks/setSchema pushed by the client on_attach.
            return
        end

        if method == "easytasks/setSchema" then
        ...
        if debug_commands then
            if method == "easytasks/dumpCst"
                ...
                local ctx = documents[uri]
                local text
                if method == "easytasks/dumpCst" then
                    text = dump_cst(ctx)
