...
--------------------------------------------------------------------------------
-- Schema navigation (mirrors schema_mapper._populate logic)
--------------------------------------------------------------------------------

-- Merge allOf, resolve if/then/else and oneOf branches using `data`.
-- Returns a new flat schema table without conditional keys.
---@param s table
---@param d any
---@return table
local function flatten(s, d)
    local out = {}
    utils.deep_merge_tables(out, s)

    if s.allOf then
        for _, sub in ipairs(s.allOf) do
            utils.deep_merge_tables(out, flatten(sub, d))
        end
    end
    ...
    if s["if"] then
        ...
        if ok and s["then"] then
            utils.deep_merge_tables(out, flatten(s["then"], d))
        elseif not ok and s["else"] then
            utils.deep_merge_tables(out, flatten(s["else"], d))
        end
    end
    ...
    if s.oneOf then
        ...
        for _, sub in ipairs(s.oneOf) do
            ...
            if #errs < best_n then best_n = #errs; best = sub end
            if best_n == 0 then break end
        end
        if best then utils.deep_merge_tables(out, flatten(best, d)) end
    end
...
---@param path        string
---@return table?
local function schema_at(root_schema, root_data, path)
    local parts = utils.split_path(path)
    local s, d = root_schema, root_data

    for _, seg in ipairs(parts) do
        local flat = flatten(s, d)
...
function M.handler(context, params, callback)
    ...
    if before:match("=") then
        ...
        local flat  = schema_at(context.schema, context.data, kpath)
    ...
    local flat     = schema_at(context.schema, context.data, spath)
