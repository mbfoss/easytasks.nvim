...
-- over a pair of anonymous pipes. Pass `require("easytasks.lsp.thread.thread_client").start`
...
function M.start(dispatchers)
    ...
    local function safe_on_message(msg)
        vim.schedule(function()
            ...
            if not ok then
                vim.notify("[easytasks] thread client message error: " .. tostring(err), vim.log.levels.ERROR)
            end
        end)
    end
    ...
    local thread_main = require("easytasks.lsp.server.thread").thread_main
