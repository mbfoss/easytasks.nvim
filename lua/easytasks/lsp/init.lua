...
-- supported here.
local thread_client = require("easytasks.lsp.client")
...
--M.diagnostics_ns = vim.api.nvim_create_namespace("easytasks-toml")
...
---@param buf  integer
---@param opts easytasks.ThreadLspStartOpts?
...
function M.dump(buf, what)
    local entry = attached[buf]
    if not entry then
        vim.notify("[easytasks] no LSP client attached to buffer " .. tostring(buf), vim.log.levels.WARN)
        return
    end
    if not entry.debug_commands then
        vim.notify("[easytasks] debug_commands not enabled for this buffer", vim.log.levels.WARN)
        return
    end

    local method = _dump_methods[what]
    if not method then
        vim.notify("[easytasks] unknown dump target: " .. tostring(what), vim.log.levels.ERROR)
        return
    end
    ...
    local client = vim.lsp.get_client_by_id(entry.client_id)
    if not client then
        vim.notify("[easytasks] LSP client not found", vim.log.levels.ERROR)
        return
    end
    client:request(method --[[@as any]], params, function(err, result)
        if err then
            vim.notify("[easytasks] dump error: " .. tostring(err.message), vim.log.levels.ERROR)
            return
        end
        ...
        vim.api.nvim_buf_set_name(scratch, "[easytasks:" .. what .. "]")
