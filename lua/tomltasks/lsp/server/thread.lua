-- tomltasks LSP server — in-process alternative to server.lua.
...
-- module must not reference any tomltasks.lsp.thread.thread_server file-level local.
local M = {}
...
function M.thread_main(read_fd, write_fd)
    ...
    require("tomltasks.lsp.server.compact").install()
    ...
    local hover       = require("tomltasks.lsp.server.hover")
    ...
    local function log(msg, level)
        write_frame({
            ...
            params = { type = level or MessageType.Log, message = "[tomltasks] " .. tostring(msg) },
        })
    end
    ...
    ---@param uri  string
    ---@param text string
    ---@return tomltasks.LspBufferContext
    ...
    local function dispatch(msg)
        ...
        if method == "textDocument/didOpen" then
            ...
            doc_text[uri] = text
            -- Schema arrives via tomltasks/setSchema pushed by the client on_attach.
            return
        end

        if method == "tomltasks/setSchema" then
        ...
        if debug_commands then
            if method == "tomltasks/dumpCst"
                ...
                or method == "tomltasks/dumpData"
                or method == "tomltasks/dumpSchema" then
                ...
                local ctx = documents[uri]
                local text
                if method == "tomltasks/dumpCst" then
                    text = dump_cst(ctx)
                elseif method == "tomltasks/dumpDecodeTree" then
                    text = dump_decode_tree(ctx)
                elseif method == "tomltasks/dumpData" then
                    text = dump_data(ctx)
                else
