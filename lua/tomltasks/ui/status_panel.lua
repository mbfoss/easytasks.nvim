...
local throttle                  = require('tomltasks.tk.throttle')
...
local _run_map                  = {} ---@type table<string, tomltasks.RunEntry>
...
local _log_sub                  = nil ---@type tomltasks.LogSub?
...
local function _setup_hl()
    ...
    vim.api.nvim_set_hl(0, "TomlTasksActiveTab", { fg = fg("Title"), bg = bg("WinBar"), bold = true, default = true })
    vim.api.nvim_set_hl(0, "TomlTasksBadgeOk", { link = "DiagnosticOk", default = true })
    vim.api.nvim_set_hl(0, "TomlTasksBadgeErr", { link = "DiagnosticError", default = true })
    vim.api.nvim_set_hl(0, "TomlTasksBadgeWarn", { link = "DiagnosticWarn", default = true })
    vim.api.nvim_set_hl(0, "TomlTasksBadgeHint", { link = "DiagnosticHint", default = true })
    vim.api.nvim_set_hl(0, "TomlTasksBadgeMuted", { link = "WinBar", default = true })
    vim.api.nvim_set_hl(0, "TomlTasksUnread", { link = "DiagnosticHint", default = true })
end

---@type table<tomltasks.TaskState, {icon:string, hl:string}>
local _badge = {
    running = { icon = "▶", hl = "TomlTasksBadgeOk" },
    waiting = { icon = "⧗", hl = "TomlTasksBadgeWarn" },
    ok      = { icon = "✓", hl = "TomlTasksBadgeOk" },
    failed  = { icon = "✗", hl = "TomlTasksBadgeErr" },
...
local function _refresh_log_buf(entry, run_id)
    if not _log_buf then
        ...
        vim.api.nvim_buf_set_var(_log_buf, "tomltasks_autoscroll", true)
    end
...
local function _get_empty_buf()
    if not _empty_buf or not vim.api.nvim_buf_is_valid(_empty_buf) then
        local ns = vim.api.nvim_create_namespace("TomlTasksEmpty")
...
local function _build_winbar(width)
    if #_runs == 0 then
        return "%#WinBar# %#TomlTasksBadgeMuted#No tasks%#WinBar#"
    end
    ...
    for run_idx, run_id in ipairs(_runs) do
        ...
        if #entry.bufnrs > 0 and not entry.is_shell then
            ...
            for pi, be in ipairs(entry.bufnrs) do
                ...
                if has_unread then
                    if is_cur then
                        part = string.format(
                            "%%%d@v:lua._TomlTasksWbc@%s%%#TomlTasksUnread#•%s%%X",
                            page_num, lbl, tab_hl)
                    else
                        part = string.format(
                            "%%%d@v:lua._TomlTasksWbc@%%#TomlTasksBadgeMuted#%s%%#TomlTasksUnread#•%s%%X",
                            page_num, lbl, tab_hl)
                    end
                elseif is_cur then
                    part = string.format(
                        "%%%d@v:lua._TomlTasksWbc@%s%%X", page_num, lbl)
                else
                    part = string.format(
                        "%%%d@v:lua._TomlTasksWbc@%%#TomlTasksBadgeMuted#%s%s%%X",
                        page_num, lbl, tab_hl)
                end
                parts[#parts + 1] = part
            end
        ...
        if run_idx > 1 then
            push(3, "%#TomlTasksBadgeMuted#")
            push(2, "│")
        end
        push(2, " ")
        push(3, string.format("%%%d@v:lua._TomlTasksWbc@", name_num))
        push(3, "%#" .. b.hl .. "#")
        push(2, b.icon .. " ")
        push(3, tab_hl)
        push(2, name_num .. ":")
...
function M.jump(n)
    ...
    if not n then
        require("tomltasks.ui").notify_warning("page number required")
        return
    end
