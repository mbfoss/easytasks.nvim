...
local save_buffers = require("tomltasks.util.save_buffers")
local project      = require("tomltasks.project")

---@class tomltasks.TaskTemplate
...
---@class tomltasks.ProgressEvent
---@field time    integer  unix timestamp
---@field message string

---Presentation options for a buffer registered with the run host.
---@class tomltasks.AddBufOpts
---@field label?      string
---@field priority?   integer

---@class tomltasks.RunCtx
---@field name       string   the task's name (the `[tasks.<name>]` key)
---@field run_id     string   this run's unique id (`<name>#<counter>`)
---@field add_bufnr  fun(bufnr: integer, opts?: tomltasks.AddBufOpts)
---@field report     fun(message: string)

---@alias tomltasks.TaskState "idle"|"running"|"waiting"|"ok"|"failed"|"stopped"

---@class tomltasks.RunEntry
---@field task_name      string
---@field task_type      string?
---@field state          tomltasks.TaskState
---@field waiting_for    string[]?
---@field reports        tomltasks.ProgressEvent[]
...
---@type table<string, tomltasks.RunEntry>
local _running     = {}
...
---@param fn fun(run_id: string, entry: tomltasks.RunEntry)
---@return fun() cancel
function M.on_state_change(fn) return _on_state_change:subscribe(fn) end

---@param fn fun(run_id: string, event: tomltasks.ProgressEvent)
---@return fun() cancel
...
---@return table<string, tomltasks.RunEntry>
function M.get_all()
...
local function _load_tasks(toml_path)
    ...
    do
        local expressions_reg = require("tomltasks.expressions")
...
---@param value any
---@return tomltasks.SaveBuffersConfig?
...
function M.list_expression_names(toml_path)
    local names, seen = {}, {}
    for _, entry in ipairs(require("tomltasks.expressions").list()) do
