...
---@alias tomltasks.ExpressionFn fun(ctx: tomltasks.ExpressionCtx, ...): any, string?
...
function _expressions.projectdir(_, resolve)
    ...
    local tasks_file = vim.fs.joinpath(cwd, require("tomltasks.config").tasks_filename)
...
function M.register(name, fn, opts)
    if _builtin[name] then
        error("tomltasks: cannot override built-in expression '" .. name .. "'", 2)
    end
